...
A streak length of 1 means there is 1 hit and then a miss. Therefore, the streak length is 1. A streak length of 0 means the shoot is a miss and the one before this is also a miss. That is why the streak length is 0.
...
```{r}
kobe_streak %>%
  summarise(longest_streak = max(length))

kobe_streak %>%
  count(length)

```
Most streaks have length 0, followed by 1, while longer streaks occur less frequently. The typical streak length is 0, and the longest streak is 4 baskets.
...
sim_streak %>%
  summarise(longest_simstreak = max(length))

sim_streak %>%
  count(length)

```
The typical streak length is 0 because it frequently occurs and the longest is the player is 4.
...
**Insert your answer here**
Both distribution has longer steaks on 0 and 1. Kobe's streak distribution is very similar to the simulated independent shooter, therefore, the hot hand model doesn't fit on Kobe's shooting patterns. If the hot hand were present, more long streaks of Hits than the independent shooter simulation.  
* * *
