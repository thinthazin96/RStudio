...
Based on the analysis of all three histograms, the histogram with no bin width and the one with a bin width of 15 provide a better representation of the data distribution. In the histogram with a bin width of 15, both delayed and early departures are clearly visible, unlike in the histograms with no specified bin width and with a bin width of 150. Although the histogram with a bin width of 150 shows flight delay counts exceeding 30,000, the distribution is difficult to interpret due to the wide bins.
...
**Insert your answer here**
```{r lowest-mean-departure-delay}
ggplot(data = nycflights, aes(x = dep_delay)) +
geom_histogram(binwidth = 50)

nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = min(mean(dep_delay)), n_flights = n()) %>%
  arrange((mean_dd))

nycflights %>%
  group_by(month) %>%
  summarise(median_dd = min(median(dep_delay)),n_flights = n()) %>%
  arrange((median_dd))
```
Using the lowest mean departure delay, October is the month that minimizes your potential departure delay when leaving NYC. Using the lowest median departure delay, both September and October minimize your potential delay. Pro of lowest mean delay, minimized the average delay overall and takes into account all flights, including extreme  delays.It is good for the overall system performace. But the downside is, a few very large delays can effect the mean. SO it doesn't represent a typical flight experiences.
On the other hand, the pro of the lowest median delay pick the middle one by lining up all the waiting times. So it's represent a typical flight experiences. The downside is, it ignores how severe extreme delays are. Therefore, a month could have a low median but sitll have accasional massive delays.

Since the distribution is positively skewed, the median better reflects a typical flight experience. Overall, October is the best month to minimize your potential departure delay from NYC.
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = (sum(dep_type == "on time") / n())) %>%
  summarise(Airport = min(origin), most_onTime_percentage=min(ot_dep_rate))
```
I would choose EWR airport base on the highest on time departure percentage. 
