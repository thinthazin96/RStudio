...
**Insert your answer here**
No, the data do not appear nearly normal because the distribution is right-skewed with a long tail toward larger values.
...
```
No, all of the points fall on the line. The simulated data is more normal than the real data because the points are closer to the line compare to the real data which is far away from the line. That means the real data is more skewed.
...
**Insert your answer here**
Question 1: What is the probability that a menu item has more than 500 calories?
Theoretical (normal distribution)
```{r}
1 - pnorm(500, mean = mcmean, sd = mcsd)
...
Question 2: What is the probability that a menu item has between 200 and 400 calories?
```{r}
pnorm(400, mean = mcmean, sd = mcsd) - pnorm(200, mean = mcmean, sd = mcsd)
...
ffmean <- mean(fastfood$sodium)
ffsd <- sd(fastfood$sodium)


ggplot(data = fastfood, aes(x = sodium))+
  geom_blank() +
  geom_histogram(aes(y = ..density..)) +
  stat_function(fun = dnorm, args = c(mean = ffmean, sd = ffsd), col = "tomato")
...
```{r}
sim_norm <- rnorm(n = nrow(fastfood), mean = ffmean, sd = ffsd)
...
```
Burger Kings and Taco ball has the closest to normal distribution base on the chart.
...
**Insert your answer here**
Some of the normal probability plots for sodium distribution seem to have a step wise pattern because these restaurants likely have repeated sodium values across different menu items.
...
**Insert your answer here**
```{r}
# Pick a restaurant, e.g. Burger King
burgerking <- fastfood %>%
filter(restaurant == "Burger King")

# Q-Q plot
ggplot(data = burgerking, aes(sample = total_carb)) +
  geom_qq() +
  geom_qq_line()

bkmean <- mean(burgerking$total_carb)
bksd <- sd(burgerking$total_carb)

# Histogram to confirm
ggplot(data = burgerking, aes(x = total_carb)) +
  geom_blank() +
  geom_histogram(aes(y = ..density..)) +
  stat_function(fun = dnorm, args = c(mean = bkmean, sd = bksd), col = "tomato")
  
```
The total_carb variable is slightly right_skewed, and the histogram confirms what would be seen in the normal probability plot.
