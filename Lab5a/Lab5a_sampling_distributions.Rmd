...
ggplot(samp1, aes(x = scientist_work)) +
  ...
  labs(
    x = "", y = "",
    title = "Do you believe that the work scientists do benefit people like you (50 sample)?"
  ) +
  coord_flip() 
```

The sample distribution will be similar but not identical to the population. In the population, 80% say "Benefits" and 20% say "Doesn't benefit." In the sample of 50, the proportions vary slightly 74% vs 26% and the difference is due to random sampling variability. Overall, the sample still roughly reflects the population but is not exact.
...
**Insert your answer here**
No, the sample proportions will not match exactly because each student takes a different random sample. However, they should be somewhat similar since all samples come from the same population.
...
samp4 %>%
  ...
  mutate(p_hat = n / sum(n))
```
The sample proportions from samp1 and samp2 are different but close. As we can see, the larger the sample size, the better the accuracy. 
...
**Insert your answer here**
There are 15,000 observations which is one for each sample and each of them represents a sample proportion p_hat. This sampling distribution has normal(bell-shaped) distribution, the center is around 0.20, and the spread moderate variablity due to sample size of 50.
